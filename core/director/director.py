...
_ACCEPTANCE_PROMPT = """你是 Echelon 框架的 Director，Squad 刚刚完成了一项任务，请你给用户一份简短验收总结。
...
要求：不要粘贴长文件内容，不要输出冗长报告，总字数控制在 300 字以内。"""
...
class DirectorAgent:
    ...
    async def accept(self, task: str, file_list: str) -> str:
        ...
        try:
            ...
            return resp.get("content", "验收完成")[:600]
        except Exception as e:
            return f"验收失败：{e}"

    async def monitor(self, status: dict) -> str:
        """执行者 Director 定期查看 Squad 状态并给用户简短报告。"""
        if self.role != "executor":
            return ""
        prompt = (
            "你是执行者 Director。请基于以下 Squad 状态，给用户一条简短进度报告，"
            "只说明当前进度、风险和下一步，不要猜测模型身份。\n\n"
