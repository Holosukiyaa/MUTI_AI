...
def _list_partners() -> list[dict]:
    if not os.path.exists(PARTNERS_DIR):
        return []
    result = []
    for name in sorted(os.listdir(PARTNERS_DIR)):
        cfg_path = os.path.join(PARTNERS_DIR, name, "config.json")
        if os.path.isfile(cfg_path):
            try:
                with open(cfg_path, encoding="utf-8") as f:
                    d = json.load(f)
                    d["_name"] = name
                    d["_dir"] = os.path.join(PARTNERS_DIR, name)
                    result.append(d)
            except Exception:
                pass
    return result
...
def _create_partner():
    os.system('cls')
    console.print()
    console.print(Rule("[bold cyan]新建 Partner[/bold cyan]", style="cyan"))
    console.print()
    ...
    console.print(f"\n  [green]✓ 已创建 Partner：{name}[/green]  按任意键继续...")
    msvcrt.getwch()


def _delete_partner(partner: dict):
    name = partner["_name"]
    action = _menu(f"确认删除 {name}？", ["取消", "确认删除（不可恢复）"])
    if action == 1:
        shutil.rmtree(partner["_dir"], ignore_errors=True)
        os.system('cls')
        console.print(f"\n  [green]✓ 已删除 {name}[/green]  按任意键继续...")
        msvcrt.getwch()


def run_partner_ui() -> dict | None:
    """返回选中的 partner dict，或 None（返回主菜单）。"""
    while True:
        partners = _list_partners()
        labels = [f"{p['_name']}  —  {p.get('description', '')}" for p in partners]
        labels += ["＋  新建 Partner", "← 返回主菜单"]

        sel = _menu("Partner 管理", labels)
        ...
        partner = partners[sel]
        action = _menu(f"Partner · {partner['_name']}", ["启动任务会话", "删除", "← 返回"])
        if action == 0:
            return partner
        elif action == 1:
            _delete_partner(partner)
